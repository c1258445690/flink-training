...
public class RidesAndFaresExercise {
    ...
    public static class EnrichmentFunction
        ...
        private ValueState<RideAndFare> valueState;

        @Override
        public void open(Configuration config) throws Exception {
            valueState =
                    getRuntimeContext().getState(
                            new ValueStateDescriptor<>("valueState", RideAndFare.class));
        }

        @Override
        public void flatMap1(TaxiRide ride, Collector<RideAndFare> out) throws Exception {
            if (valueState.value() == null) {
                valueState.update(new RideAndFare());
            }
            valueState.value().ride = ride;
            if (valueState.value().fare != null) {
        ...
        public void flatMap2(TaxiFare fare, Collector<RideAndFare> out) throws Exception {
            ...
            valueState.value().fare = fare;
            if (valueState.value().ride != null) {
                out.collect(valueState.value());
                valueState.clear();
            }
        }
